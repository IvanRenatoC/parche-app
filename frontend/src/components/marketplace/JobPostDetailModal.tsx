...
import { getJobPostApplications, withdrawApplication, applyToJobPost, closeJobPost } from '../../services/jobPosts';
...
export function JobPostDetailModal({ post, isOwner, onClose, onUpdated }: Props) {
  ...
  useEffect(() => {
    if (isOwner) {
      setLoadingApps(true);
      getJobPostApplications(post.id).then(apps => {
        setApplications(apps);
        setLoadingApps(false);
      });
    } else if (appUser) {
      getJobPostApplications(post.id).then(apps => {
        const mine = apps.find(a => a.worker_uid === appUser.uid);
        setMyApplication(mine ?? null);
      });
    }
  }, [post.id, isOwner, appUser]);
  ...
  async function handleClose() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Error al cerrar publicación');
    } finally {
      setActionLoading(null);
    }
  }
  ...
  async function handleAcceptWorker(application: Application) {
    ...
    try {
      ...
      const updated = await getJobPostApplications(post.id);
      setApplications(updated);
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Error al aceptar postulante');
    } finally {
      setActionLoading(null);
    }
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            ...
            <h2 style={{ fontSize: '20px', fontWeight: 700, color: '#111', margin: '0 0 4px' }}>{post.title}</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          ...
          <MetaItem icon={<Users size={14} />} label="Workers">{post.accepted_workers_count} / {post.required_workers} cubiertos</MetaItem>
          <MetaItem icon={<Briefcase size={14} />} label="Oficio">{post.occupation}</MetaItem>
        </div>

        {/* Description */}
        <div>
          <h4 style={{ fontSize: '14px', fontWeight: 600, color: '#374151', marginBottom: '6px' }}>Descripción</h4>
          <p style={{ fontSize: '14px', color: '#6b7280', lineHeight: 1.6 }}>{post.description}</p>
        </div>

        {post.requirements && (
          <div>
            <h4 style={{ fontSize: '14px', fontWeight: 600, color: '#374151', marginBottom: '6px' }}>Requisitos</h4>
            <p style={{ fontSize: '14px', color: '#6b7280', lineHeight: 1.6 }}>{post.requirements}</p>
          </div>
        )}
        ...
        {!isOwner && post.status === 'published' && (
          <div style={{ borderTop: '1px solid #f2f3f5', paddingTop: '16px' }}>
            {!myApplication ? (
              <Button fullWidth onClick={handleApply} loading={actionLoading === 'apply'}>
        ...
        {/* Owner actions */}
        {isOwner && (
          <div style={{ borderTop: '1px solid #f2f3f5', paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
...
function ApplicationRow({ application, canAccept, onAccept, loading }: {
  ...
  return (
    <div style={{
      ...
      padding: '10px 14px', borderRadius: '8px', background: '#f9fafb', border: '1px solid #f0f0f0',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 600, color: '#374151' }}>
          W
        </div>
        <div>
          <p style={{ fontSize: '13px', fontWeight: 600, color: '#111', margin: 0 }}>Worker {application.worker_uid.slice(0, 8)}...</p>
      ...
      {canAccept && (
        <Button size="sm" variant="primary" loading={loading} onClick={onAccept}>
          <LogIn size={13} style={{ marginRight: '4px' }} /> Aceptar
        </Button>
      )}
    </div>
  );
...
function MetaItem({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
      ...
      <span style={{ fontSize: '13px', color: '#374151', fontWeight: 500 }}>{children}</span>
    </div>
  );
}
